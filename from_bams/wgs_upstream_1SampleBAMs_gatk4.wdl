workflow wgs 
{
      ...
      scatter(index in range(length(bams)))
      {
          ...
          {
             input:
               fastq1               = BAMtoFASTQ.first_fastq,
	       fastq2               = BAMtoFASTQ.second_fastq,
...
{ 
  ...
  command
  {
    ${tools}/gatk-4.0.5.1/gatk --java-options "-Xms8g -XX:GCTimeLimit=50 -XX:GCHeapFreeLimit=10 -XX:ParallelGCThreads=2" HaplotypeCaller \
      -R ${ref_fasta} \
      -O ${gvcf_basename}.vcf \
      -I ${input_bam} \
      -L ${interval_list} \
...
task SortSam_byQuery
{
  ...
  command
  {
     ...
     #${sambamba} sort -m 5G -o ${output_bam_basename}.bam ${flag} -l ${compressionLvl} -t ${cpus} ${input_bam}
  }
...
task SortAndFixTags
{
  ...
  command
  {
    ...
    ${tools}/samtools-1.8/samtools sort -l 2 -@16 -O BAM -o ${input_bam}.sorted.bam ${input_bam}
...
task GatherBqsrReports 
{
  ...
  command
  {
      ${tools}/gatk-4.0.5.1/gatk --java-options "-Xms3000m -XX:ParallelGCThreads=2"  GatherBQSRReports \
...
task ApplyBQSR
{
  ...
  command 
  {
    ${tools}/gatk-4.0.5.1/gatk --java-options "-XX:+PrintFlagsFinal -XX:+PrintGCTimeStamps -XX:+PrintGCDateStamps \
...
task BAMtoFASTQ
{
   ...
   command
   {
    ...
    output
    {
       File first_fastq = "1.fastq"
       File second_fastq = "2.fastq"
    }
}
...
task RevertBAM
{
   ...
   command
   {
      java -Xmx5000m -jar ${picard} RevertSam \
      I=${input_bam} \
